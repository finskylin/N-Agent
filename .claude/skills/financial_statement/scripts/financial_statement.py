...
class FinancialStatementSkill:
    ...
    def description(self) -> str:
        return "三大报表（资产负债/利润/现金流）和关键财务指标"
    ...
    def input_schema(self) -> Dict[str, Any]:
        return {
            ...
            "statement_type": {"type": "string", "enum": ["资产负债表", "利润表", "现金流量表", "关键指标", "综合"], "default": "综合"},
            "periods": {"type": "integer", "default": 4, "description": "查询期数"}
        }
    ...
    async def execute(self, context: dict) -> dict:
        ...
        statement_type = params.get("statement_type", "综合")
        periods = params.get("periods", 4)

        if not code:
            return {"error": "缺少股票代码参数", "for_llm": {"error": "缺少股票代码参数"}}
        ...
        try:
            ...
            fetchers = {
                ...
                "关键指标": get_key_financial_index,
            }

            types_to_query = list(fetchers.keys()) if statement_type == "综合" else [statement_type]
            ...
            for st in types_to_query:
                ...
                df = fetcher(code)
                if df is not None and not df.empty:
                    df_limited = df.head(periods)
                    # 提取关键指标到 summary
                    self._extract_key_metrics(st, df_limited, key_metrics)
                    # 构建列定义（使用中文标签）
                    if not columns:
                        columns = self._build_columns(df_limited)
                    # 格式化数值
                    formatted_records = self._format_records(df_limited)
                    items.extend(formatted_records)
                    summary[st] = {"periods": len(df_limited)}
                    analysis_parts.append(f"{st}获取{len(df_limited)}期数据")
                    data_source = data_source or f"akshare/{st}"

            if not items:
                return {"error": f"无法获取{code}的财务报表数据", "for_llm": {"error": f"无法获取{code}的财务报表数据"}}

            # 构建摘要分析
            summary["key_metrics"] = key_metrics
            analysis = self._build_analysis(ts_code, key_metrics, analysis_parts)

            elapsed = int((datetime.now() - start_time).total_seconds() * 1000)
            return {
                "ts_code": ts_code,
                "title": f"{ts_code} 财务报表",
                "items": items[:20],
                "columns": columns,
                "summary": summary,
                "analysis": analysis,
                ...
                "for_llm": {"ts_code": ts_code, "summary": summary, "analysis": analysis}
            }
        except Exception as e:
            logger.error(f"FinancialStatementSkill execute error: {e}")
            return {"error": str(e), "for_llm": {"error": str(e)}}

    def _build_columns(self, df: pd.DataFrame) -> List[Dict[str, str]]:
        """构建列定义，限制列数并使用有意义的标签"""
        cols = []
        for col in df.columns:
            label = _KEY_METRICS_LABEL.get(col, col)
            cols.append({"key": col, "label": label})
        return cols

    def _format_records(self, df: pd.DataFrame) -> List[Dict]:
        """格式化数据记录：大数值转亿，保留2位小数"""
        records = []
        for _, row in df.iterrows():
            ...
            for col in df.columns:
                ...
                if pd.isna(val):
                    ...
                    # 大数值（>1亿）转亿
                    if abs(val) >= 1e8:
                        record[col] = f"{val / 1e8:.2f}亿"
                    elif abs(val) >= 1e4:
                        record[col] = f"{val / 1e4:.2f}万"
                    else:
    ...
    def _extract_key_metrics(self, statement_type: str, df: pd.DataFrame, metrics: Dict):
        """从报表中提取关键指标到摘要"""
        if df.empty:
            return
        latest = df.iloc[0]

        if statement_type == "关键指标":
            for key in ["每股收益", "每股净资产", "净资产收益率", "毛利率", "净利率", "资产负债率",
                        "营收同比增长", "净利润同比增长"]:
                if key in latest.index:
                    val = latest[key]
                    if pd.notna(val):
                        metrics[key] = round(float(val), 2) if isinstance(val, (int, float)) else str(val)

        elif statement_type == "利润表":
            for key in ["营业总收入", "归属净利润", "营业利润"]:
                if key in latest.index:
                    val = latest[key]
                    if pd.notna(val) and isinstance(val, (int, float)):
                        metrics[key] = f"{val / 1e8:.2f}亿"

        elif statement_type == "资产负债表":
            for key in ["总资产", "总负债", "股东权益合计"]:
                if key in latest.index:
                    val = latest[key]
                    if pd.notna(val) and isinstance(val, (int, float)):
                        metrics[key] = f"{val / 1e8:.2f}亿"
            ...
            for key in ["经营活动产生的现金流量净额", "投资活动产生的现金流量净额"]:
                if key in latest.index:
                    val = latest[key]
                    if pd.notna(val) and isinstance(val, (int, float)):
                        short_key = key.replace("产生的现金流量净额", "现金流")
                        metrics[short_key] = f"{val / 1e8:.2f}亿"

    def _build_analysis(self, ts_code: str, metrics: Dict, parts: List[str]) -> str:
        """构建财务分析文本"""
        analysis = f"{ts_code} 财务报表: " + "；".join(parts) + "。"
        ...
        if metrics:
            ...
            if roe is not None:
                if isinstance(roe, (int, float)):
                    if roe > 15:
                        highlights.append(f"ROE {roe}%，盈利能力优秀")
                    elif roe > 8:
                        highlights.append(f"ROE {roe}%，盈利能力良好")
                    else:
                        highlights.append(f"ROE {roe}%，盈利能力一般")

            debt_ratio = metrics.get("资产负债率")
            if debt_ratio is not None and isinstance(debt_ratio, (int, float)):
                if debt_ratio > 70:
                    highlights.append(f"资产负债率{debt_ratio}%，杠杆较高")
                elif debt_ratio < 40:
                    highlights.append(f"资产负债率{debt_ratio}%，财务稳健")

            if highlights:
                analysis += " 要点: " + "；".join(highlights) + "。"
...
def _main():
    ...
    parser = argparse.ArgumentParser(description="Run FinancialStatementSkill directly")
    parser.add_argument("--ts-code", type=str, dest="ts_code")
    parser.add_argument("--statement-type", type=str, dest="statement_type")
