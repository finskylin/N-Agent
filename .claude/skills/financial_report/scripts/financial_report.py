...
def get_stock_info(code: str) -> Optional[Dict[str, Any]]:
    ...
    # Fallback 1: 交易所官网 A 股列表（含行业，~1.5s）
    try:
        import akshare as ak
        with no_proxy():
            if code.startswith("6"):
                # 上交所
                df_ex = ak.stock_info_sh_name_code(symbol="主板A股")
                code_col = "SECURITY_CODE_A"
                industry_col = None  # 上交所列表无行业字段
                name_col = "SECURITY_ABBR_A"
            else:
                # 深交所
                df_ex = ak.stock_info_sz_name_code(symbol="A股列表")
                code_col = "A股代码"
                industry_col = "所属行业"
                name_col = "A股简称"
        if df_ex is not None and not df_ex.empty:
            row = df_ex[df_ex[code_col].astype(str).str.lstrip("0") == code.lstrip("0")]
            if row.empty:
                row = df_ex[df_ex[code_col].astype(str) == code]
            if not row.empty:
                r = row.iloc[0]
                raw_industry = r.get(industry_col, "") if industry_col and industry_col in r.index else ""
                # 清理行业前缀（如 "C 制造业" → "制造业"）
                industry = " ".join(raw_industry.split()[1:]) if raw_industry and raw_industry[0].isalpha() and len(raw_industry.split()) > 1 else raw_industry
                info = {
                    "股票简称": r.get(name_col, ""),
                    "行业": industry,
                }
                logger.info(f"[exchange] get_stock_info succeeded: {code}, industry={industry}")
                return info
    except Exception as e:
        logger.warning(f"[exchange] get_stock_info fallback failed: {e}")

    # Fallback 2: 雪球接口（0.3s，无行业信息，返回名称/价格）
    try:
        import akshare as ak
        prefix = "SH" if code.startswith("6") else "SZ"
        with no_proxy():
            df_xq = ak.stock_individual_spot_xq(symbol=f"{prefix}{code}")
        if df_xq is not None and not df_xq.empty:
            kv = dict(zip(df_xq["item"], df_xq["value"]))
            info = {
                "股票简称": kv.get("名称", ""),
                "最新价": kv.get("现价", 0),
                "总市值": kv.get("资产净值/总市值", 0),
                "流通市值": kv.get("流通值", 0),
            }
            logger.info(f"[xueqiu] get_stock_info succeeded: {code}")
            return info
    except Exception as e:
        logger.warning(f"[xueqiu] get_stock_info fallback failed: {e}")
...
class FinancialReportSkill:
    ...
    def description(self) -> str:
        return "获取股票财务报表摘要，包含营收、净利润、ROE、毛利率、同比/环比增长率等关键财务指标及趋势分析"
    ...
    def output_schema(self) -> Dict[str, Any]:
        return {
            "ts_code": "string",
            "reports": "list of financial data with yoy/qoq",
            "trend_analysis": "growth trend summary",
            "industry_comparison": "industry ranking"
        }

    async def execute(self, context: dict) -> dict:
        """执行财务数据获取 - 升级版，包含同比/环比、扩展指标、趋势分析"""
        ts_code = context.get("ts_code", "")
        periods = context.get("periods", 8)

        if not ts_code:
            return {"error": "缺少股票代码参数 ts_code"}
        ...
        try:
            ...
            # ========== 数据源1: 财务摘要 (stock_financial_abstract) ==========
            df_abstract = self._get_financial_abstract(code)

            # ========== 数据源2: 财务指标 (stock_financial_analysis_indicator) ==========
            df_indicator = self._get_financial_indicators(code)

            # 合并数据
            reports = self._merge_financial_data(df_abstract, df_indicator, periods, code)
            ...
            if not reports:
                return {
                    ...
                    "trend_analysis": {},
                    "industry_comparison": {},
                    "message": f"未找到 {ts_code} 的财务报表数据"
                }

            # ========== 计算同比/环比 ==========
            reports = self._calculate_yoy_qoq(reports)

            # ========== 趋势分析 ==========
            trend_analysis = self._analyze_trends(reports)

            # ========== 行业对比 (暂时返回占位数据，后续可扩展) ==========
            industry_comparison = self._get_industry_comparison(ts_code, reports)
            ...
            return {
                ...
                "message": f"成功获取 {ts_code} 的 {len(reports)} 期财务数据（含同比/环比）"
            }

        except Exception as e:
            logger.error(f"财务数据获取失败: {str(e)}", exc_info=True)
            return {"error": f"财务数据获取失败: {str(e)}"}

    def _get_financial_abstract(self, code: str) -> Optional[Any]:
        """获取财务摘要数据（akshare → adata 降级）"""
        try:
            import pandas as pd

            df = get_financial_abstract(code)
            ...
            if '指标' in df.columns:
                ...
                df = df.T
                df = df.reset_index()
                df = df.rename(columns={'index': '截止日期'})
            ...
            if '截止日期' in df.columns:
                df = df[df['截止日期'].astype(str).str.match(r'^\d{8}$')]
            elif '报告期' in df.columns:
                df = df[df['报告期'].astype(str).str.match(r'^\d{8}$')]
                df = df.rename(columns={'报告期': '截止日期'})
    ...
    def _get_financial_indicators(self, code: str) -> Optional[Any]:
        """获取财务指标数据（周转率、现金流等）- akshare → adata 降级"""
        try:
            import pandas as pd

            df = get_financial_indicators(code)
            ...
            # 类似的数据转置处理
            if '指标' in df.columns or '项目' in df.columns:
                index_col = '指标' if '指标' in df.columns else '项目'
                if '选项' in df.columns:
                    df = df.drop(columns=['选项'])
    ...
    def _merge_financial_data(self, df_abstract: Any, df_indicator: Any, periods: int, code: str = "") -> List[Dict]:
        """合并财务摘要和财务指标数据"""
        import pandas as pd

        reports = []

        if df_abstract is None:
            return reports

        limit_df = df_abstract.head(periods)

        for idx, row in limit_df.iterrows():
            date_val = self._get_val(row, ['截止日期', '截至日期', 'date'])

            report = {
                "end_date": str(date_val),
                "ann_date": "N/A",

                # 基础数据
                "revenue": self._parse_num(self._get_val(row, ['营业总收入', '营业收入', 'revenue'])),
                "net_profit": self._parse_num(self._get_val(row, ['归母净利润', '净利润', 'net_profit'])),
                "eps": self._parse_num(self._get_val(row, ['基本每股收益', 'eps'])),
                "bps": self._parse_num(self._get_val(row, ['每股净资产', 'bps'])),
                "roe": self._parse_num(self._get_val(row, ['净资产收益率(ROE)', '净资产收益率', 'roe'])),

                # 利润率指标
                "gross_margin": self._parse_num(self._get_val(row, ['销售毛利率', '毛利率', 'gross_margin'])),
                "net_margin": self._parse_num(self._get_val(row, ['销售净利率', '净利率', 'net_margin'])),
                "operating_margin": self._parse_num(self._get_val(row, ['营业利润率', 'operating_margin'])),
                "roa": self._parse_num(self._get_val(row, ['总资产报酬率', '资产报酬率', 'roa'])),

                # 偿债能力
                "debt_ratio": self._parse_num(self._get_val(row, ['资产负债率', 'debt_ratio'])),
                "current_ratio": self._parse_num(self._get_val(row, ['流动比率', 'current_ratio'])),
                "quick_ratio": self._parse_num(self._get_val(row, ['速动比率', 'quick_ratio'])),
                "interest_coverage": self._parse_num(self._get_val(row, ['利息保障倍数', '已获利息倍数', 'interest_coverage'])),

                # 运营效率
                "inventory_turnover": self._parse_num(self._get_val(row, ['存货周转率', 'inventory_turnover'])),
                "receivable_turnover": self._parse_num(self._get_val(row, ['应收账款周转率', 'receivable_turnover'])),
                "asset_turnover": self._parse_num(self._get_val(row, ['总资产周转率', 'asset_turnover'])),

                # 现金流
                "operating_cashflow": self._parse_num(self._get_val(row, ['每股经营现金流', '经营活动现金流量净额'])),
                "free_cashflow": 0.0,  # 稍后从现金流量表计算
                "capex": 0.0,  # 稍后从现金流量表计算
            ...
            if df_indicator is not None and not df_indicator.empty:
                indicator_row = self._find_matching_row(df_indicator, str(date_val))
                if indicator_row is not None:
                    report["inventory_turnover"] = self._parse_num(
                        self._get_val(indicator_row, ['存货周转率'])
                    ) or report["inventory_turnover"]
                    report["receivable_turnover"] = self._parse_num(
                        self._get_val(indicator_row, ['应收账款周转率'])
                    ) or report["receivable_turnover"]
                    report["asset_turnover"] = self._parse_num(
                        self._get_val(indicator_row, ['总资产周转率'])
                    ) or report["asset_turnover"]

            reports.append(report)

        # 从现金流量表和利润表补充 capex/free_cashflow/rd
        if code:
            self._fill_cashflow_and_rd(code, reports)

        return reports

    def _fill_cashflow_and_rd(self, code: str, reports: List[Dict]):
        """从现金流量表和利润表补充 capex/free_cashflow/rd_expense/rd_ratio"""
        # 补充现金流数据 (capex, free_cashflow)
        try:
            import pandas as pd

            cf_df = get_cashflow_statement(code)
            if cf_df is not None and not cf_df.empty:
                cf_df.columns = [str(c).strip() for c in cf_df.columns]
                for report in reports:
                    end_date = report.get('end_date', '')
                    matched = None
                    for date_col in ['报告期', '截止日期', 'REPORT_DATE', 'date']:
                        if date_col in cf_df.columns:
                            match = cf_df[cf_df[date_col].astype(str).str[:10] == end_date[:10]]
                            if not match.empty:
                                matched = match.iloc[0]
                                break
                    if matched is not None:
                        # 购建固定资产、无形资产和其他长期资产支付的现金
                        capex = abs(self._parse_num(self._get_val(matched, [
                            '购建固定资产、无形资产和其他长期资产支付的现金',
                            '购建固定资产无形资产和其他长期资产支付的现金',
                            'CONSTRUCT_LONG_ASSET_PAY',
                            '购建固定资产',
                        ])))
                        # 经营活动产生的现金流量净额
                        ocf = self._parse_num(self._get_val(matched, [
                            '经营活动产生的现金流量净额',
                            '经营活动现金流量净额',
                            'NETCASH_OPERATE',
                        ]))
                        report['capex'] = round(capex, 2)
                        report['free_cashflow'] = round(ocf - capex, 2)
                        if report['operating_cashflow'] == 0 and ocf != 0:
                            report['operating_cashflow'] = round(ocf, 2)
        except Exception as e:
            logger.warning(f"填充现金流数据失败: {e}")

        # 补充研发数据 (rd_expense, rd_ratio)
        try:
            import pandas as pd

            inc_df = get_income_statement(code)
            if inc_df is not None and not inc_df.empty:
                inc_df.columns = [str(c).strip() for c in inc_df.columns]
                for report in reports:
                    end_date = report.get('end_date', '')
                    matched = None
                    for date_col in ['报告期', '截止日期', 'REPORT_DATE', 'date']:
                        if date_col in inc_df.columns:
                            match = inc_df[inc_df[date_col].astype(str).str[:10] == end_date[:10]]
                            if not match.empty:
                                matched = match.iloc[0]
                                break
                    if matched is not None:
                        rd = self._parse_num(self._get_val(matched, [
                            '研发费用', '研发支出', 'RESEARCH_EXPENSE', 'rd_expense',
                        ]))
                        revenue = report.get('revenue', 0)
                        report['rd_expense'] = round(rd, 2)
                        report['rd_ratio'] = round(rd / revenue * 100, 2) if revenue > 0 else 0.0
        except Exception as e:
            logger.warning(f"填充研发数据失败: {e}")

    def _calculate_yoy_qoq(self, reports: List[Dict]) -> List[Dict]:
        """计算同比和环比增长率"""
        if len(reports) < 2:
            return reports

        # 按日期排序（最新在前）
        reports = sorted(reports, key=lambda x: x.get('end_date', ''), reverse=True)

        for i, report in enumerate(reports):
            current_date = report.get('end_date', '')

            # 计算环比 (与上一期比较)
            if i + 1 < len(reports):
                prev_report = reports[i + 1]
                report['revenue_qoq'] = self._calc_growth_rate(
                    report.get('revenue', 0), prev_report.get('revenue', 0)
                )
                report['profit_qoq'] = self._calc_growth_rate(
                    report.get('net_profit', 0), prev_report.get('net_profit', 0)
                )

            # 计算同比 (与去年同期比较)
            yoy_date = self._get_yoy_date(current_date)
            yoy_report = self._find_report_by_date(reports, yoy_date)
            if yoy_report:
                report['revenue_yoy'] = self._calc_growth_rate(
                    report.get('revenue', 0), yoy_report.get('revenue', 0)
                )
                report['profit_yoy'] = self._calc_growth_rate(
                    report.get('net_profit', 0), yoy_report.get('net_profit', 0)
                )
    ...
    def _analyze_trends(self, reports: List[Dict]) -> Dict[str, Any]:
        ...
        if not reports or len(reports) < 2:
            return {
                ...
                "profit_trend": "数据不足",
                "margin_trend": "数据不足",
                "cashflow_health": "数据不足",
                "growth_quality": 0
            }

        # 获取最近几期的增长率
        recent_revenue_yoy = [r.get('revenue_yoy', 0) for r in reports[:4] if r.get('revenue_yoy')]
        recent_profit_yoy = [r.get('profit_yoy', 0) for r in reports[:4] if r.get('profit_yoy')]
        recent_margin = [r.get('gross_margin', 0) for r in reports[:4] if r.get('gross_margin')]

        # 营收趋势
        revenue_trend = self._determine_trend(recent_revenue_yoy)

        # 利润趋势
        profit_trend = self._determine_trend(recent_profit_yoy)

        # 毛利率趋势
        margin_trend = self._determine_margin_trend(recent_margin)

        # 现金流健康度
        latest = reports[0]
        cashflow_health = self._assess_cashflow_health(latest)

        # 成长质量评分 (0-100)
        growth_quality = self._calc_growth_quality(reports)
    ...
    def _get_industry_comparison(self, ts_code: str, reports: List[Dict]) -> Dict[str, Any]:
        """获取行业对比 - 使用真实行业数据"""
        if not reports:
            return {
                "industry": "未知",
        ...
        growth = latest.get('revenue_yoy', 0)
        code = ts_code.split('.')[0]
        ...
        try:
            ...
            if info:
                industry_name = info.get('行业', '') or info.get('industry', '') or '未知'
        except Exception as e:
            logger.warning(f"获取行业名称失败: {e}")

        # get_all_stocks_realtime 返回行情数据无行业列，直接走阈值估算

        # Fallback: 如果排名仍为 N/A，使用阈值估计
        if roe_rank == "N/A":
            roe_rank = "前10%" if roe > 20 else ("前30%" if roe > 10 else "中等")
        if margin_rank == "N/A":
            margin_rank = "前10%" if margin > 50 else ("前30%" if margin > 30 else "中等")
        if growth_rank == "N/A":
            growth_rank = "前10%" if growth > 30 else ("前30%" if growth > 15 else "中等")
    ...
    def _get_val(self, row, keys: List[str]):
        """从行数据中获取值"""
        import pandas as pd
        for k in keys:
            if k in row:
    ...
    def _parse_num(self, val) -> float:
        """解析数值"""
        import pandas as pd
        if pd.isna(val) or val == 'nan' or val == '' or val == '-':
            return 0.0
    ...
    def _find_matching_row(self, df, date_str: str):
        """在DataFrame中查找匹配日期的行"""
        import pandas as pd
        if df is None or df.empty:
            return None

        for col in ['截止日期', '报告期', 'date']:
            if col in df.columns:
                matching = df[df[col].astype(str) == date_str]
    ...
    def _get_yoy_date(self, date_str: str) -> str:
        """获取去年同期日期"""
        if not date_str or len(date_str) != 8:
            return ""
        try:
            year = int(date_str[:4])
            return str(year - 1) + date_str[4:]
        except:
            return ""

    def _find_report_by_date(self, reports: List[Dict], date_str: str) -> Optional[Dict]:
        """根据日期查找报告"""
        for r in reports:
            if r.get('end_date') == date_str:
                return r
        return None
    ...
    def _determine_trend(self, growth_rates: List[float]) -> str:
        ...
        if len(growth_rates) >= 2:
            trend_direction = growth_rates[0] - growth_rates[-1]
        else:
            trend_direction = 0

        if avg > 20 and trend_direction > 0:
            return "加速增长"
        elif avg > 10:
            return "稳定增长"
        elif avg > 0:
    ...
    def _determine_margin_trend(self, margins: List[float]) -> str:
        """判断利润率趋势"""
    ...
    def _assess_cashflow_health(self, report: Dict) -> str:
        """评估现金流健康度"""
        cashflow = report.get('operating_cashflow', 0)
        ...
        # 经营现金流/净利润比率
        ratio = cashflow / profit if profit > 0 else 0
    ...
    def _calc_growth_quality(self, reports: List[Dict]) -> int:
        """计算成长质量评分 (0-100)"""
        if not reports:
            return 0
...
def _main():
    ...
    parser = argparse.ArgumentParser(description="Run FinancialReportSkill directly")
