...
class DocxPreviewSkill:
    ...
    def description(self) -> str:
        return "生成或预览 Word 文档 (.docx)。支持从数据生成文档，或预览现有文档，提供下载链接和文本预览。"
    ...
    def input_schema(self) -> Dict[str, Any]:
        return {
            "file_path": {"type": "string", "required": False, "description": "现有 docx 文件路径 (预览模式)"},
            "data": {
                "type": "object", 
                "required": False, 
                "description": "用于生成文档的数据 (生成模式)。格式: {'title': str, 'sections': [{'heading': str, 'content': str}]}"
            },
            "file_url": {"type": "string", "required": False, "description": "docx 文件 URL"},
    ...
    async def _extract_text_from_docx(self, file_path: str) -> str:
        ...
        try:
            ...
            print(f"[DocxPreview] Text extraction failed: {e}")
            return ""
    ...
    async def _read_file_as_base64(self, file_path: str) -> tuple[str, int]:
        ...
        try:
            ...
            print(f"[DocxPreview] Failed to read file: {e}")
            return "", 0
    ...
    async def execute(self, context: dict) -> dict:
        ...
        try:
            ...
            # 模式 2: URL 预览模式（下载远程文件到本地）
                ...
                try:
                    dl_result = _download_remote_file(file_url, allowed_extensions=['.docx'])
                    file_path = dl_result["file_path"]
                    download_url = dl_result["download_url"]
                    print(f"[DocxPreview] Downloaded remote file: {file_url} -> {file_path}")
                except Exception as e:
                    return SkillResult(
                        status=SkillStatus.ERROR,
            ...
            # 读取文件为 base64（用于前端下载/预览）
            file_data_base64, file_size = await self._read_file_as_base64(file_path)
            
            # 如果是现有文件但没有生成 download_url，构造一个本地引用 url (如果适用)
            # 这里如果不生成，通常前端无法下载 random absolute path。
            # FileGenerator 生成的文件在 /app/data/generated，可以下载。
            # 外部传入的 path 如果不在 generated，可能无法直接下载。
            # 我们假设 FileGenerator 逻辑覆盖了主要用例。
            ...
            return SkillResult(
                ...
                data=data,
                message=f"Successfully processed Word document: {file_name}. Please render the UI using [UI_RENDER: office_preview, docx_preview]"
            )
...
def _main():
    ...
    parser = argparse.ArgumentParser(description="Run DocxPreviewSkill directly")
