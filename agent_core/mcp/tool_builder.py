...
AgentCore 版本:
- 从 agent_core 内部导入 SkillDiscovery, V4SkillExecutor, V4Config
...
from ..skill_executor import V4SkillExecutor
...
class MCPToolBuilder:
    ...
    def build_mcp_servers(self, only_skills: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # 构建 stdio MCP server 配置
        # 优先使用 agent_core/mcp/ 下的 stdio_server.py
        server_script = str(
            Path(__file__).parent / "stdio_server.py"
        )

        # 在容器中使用绝对路径（fallback）
        if not Path(server_script).exists():
            server_script = "/app/app/agent/v4/stdio_mcp_server.py"
